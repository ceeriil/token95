...
export class Plinko {
  ...
  simulate(desiredBucketIndex: number) {
    ...
    return finalResults.filter(
      ({ bucketIndex }) => bucketIndex === desiredBucketIndex
    );
  }
  ...
  collisionHandler = (event: Matter.IEventCollision<Matter.Engine>) => {
    ...
    if (this.props.onContact) {
      this.props.onContact(contactEvent);
    }
  };
  ...
  run(desiredMultiplier: number) {
    ...
    if (!candidates.length)
      throw new Error("Failed to simulate desired outcome");
    ...
    const chosenCollisions = chosen.collisions.filter(({ event }) => {
      return (
        event.plinko && event.plinko.plugin.startPositionIndex === chosenIndex
      );
    });
  ...
  private startReplayAnimation() {
    ...
    const animate = () => {
      ...
      const frameCollisions = this.replayCollisions.filter(
        (c) => c.frame === this.currentFrame
      );
