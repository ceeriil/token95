...
const Wrapper = styled.div<{ $isMinimized: boolean }>`
  ...
  background: ${({ $isMinimized }) =>
    $isMinimized ? "#5e47ff" : "rgba(28,28,35,0.85)"};
  border: 1px solid
    ${({ $isMinimized }) =>
      $isMinimized ? "rgba(255,255,255,0.3)" : "rgba(255,255,255,0.1)"};
  color: #eee;
  font-size: 0.9rem;
    ...
    <Wrapper $isMinimized={isMinimized}>
      ...
      <ContentContainer $isMinimized={isMinimized}>
        ...
        <Log ref={logRef}>
          ...
          {error && (
            <LoadingText style={{ color: "#ff8080" }}>
              Error loading chat.
            </LoadingText>
          )}
          {messages.map((m, i) => (
        ...
        <InputRow>
          <TextInput
            ...
            value={text}
            placeholder={
              connected ? "Say something…" : "Connect wallet to chat"
            }
          ...
          <SendBtn
            onClick={send}
            disabled={
              !connected || isSending || cooldown > 0 || !text.trim() || !swrKey
            }
          >
